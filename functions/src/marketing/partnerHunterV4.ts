...
const instantlyCampaignKdv = defineSecret('INSTANTLY_CAMPAIGN_KDV'); // New: segment-specific campaigns
const instantlyCampaignSchool = defineSecret('INSTANTLY_CAMPAIGN_SCHOOL');
const instantlyCampaignPro = defineSecret('INSTANTLY_CAMPAIGN_PRO');
...
export const partnerHunterV4 = functions.scheduler.onSchedule({
 ...
 */
async function fase1_discover(city: string, searchTerm: string) {
    ...
    const response = await axios.post(
        ...
        [{
            location_code: locationCode,  // Fixed: use location_code
            keyword: searchTerm,
            language_code: "nl"
        }],
        {
 ...
 */
function fase2_verify(item: any, searchTerm: string) {
    const url = item.url || '';
    ...
    // Determine segment
    let segment: 'kdv_bso' | 'school' | 'pro' | 'creator' = 'kdv_bso';
 ...
 */
function extractFactPack(html: string): string[] {
    const $ = cheerio.load(html);
    const facts: string[] = [];
...
async function fase5_sequence(lead: any) {
    ...
    const CAMPAIGN_MAP: Record<string, any> = {
    ...
    const payload = {
        campaign_id: campaignSecret.value(),  // Fixed: use campaign_id for bulk endpoint

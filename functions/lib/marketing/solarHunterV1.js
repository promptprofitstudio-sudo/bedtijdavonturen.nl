...
const DRY_RUN = (0, params_1.defineBoolean)('SOLAR_HUNTER_DRY_RUN', {
    ...
    for (const item of discoveries) {
        try {
            ...
            if (verification.status === 'rejected') {
                ...
                await saveLead(db, Object.assign(Object.assign({}, verification), { location,
                    state,
 ...
 */
function fase2_verify(item, searchQuery) {
    var _a, _b;
    const url = item.url || '';
...
async function fase3_enrich(lead) {
    ...
    try {
        ...
        if (hunterRes.data.data.emails && hunterRes.data.data.emails.length > 0) {
            ...
            const verifyRes = await axios_1.default.get(`https://api.hunter.io/v2/email-verifier?email=${email.value}&api_key=${hunterApiKey.value()}`);
    ...
    if (html) {
        ...
        if (mailtoLinks.length > 0) {
            ...
            if (email && email.includes('@')) {
                ...
                try {
                    const verifyRes = await axios_1.default.get(`https://api.hunter.io/v2/email-verifier?email=${email}&api_key=${hunterApiKey.value()}`);
                    const hunterScore = verifyRes.data.data.score || 0;
                    if (hunterScore >= 70) {
 ...
 */
function extractSolarFactPack(html) {
    const $ = cheerio.load(html);
    const facts = [];
    ...
    const payload = {
        ...
        leads: [{
                ...
                custom_variables: {
                    ...
                    hunter_score: ((_b = lead.enrichmentData.hunterScore) === null || _b === void 0 ? void 0 : _b.toString()) || '',
